#![expect(
    ...
    reason = "the CSV formatter keeps explicit slot indices to preserve output ordering"
)]
...
impl OutputFormatter {
    ...
    pub fn write_scheduling_solution(solution: &Solution) -> crate::Result<FormattedOutput> {
        let scheduling = solution
            .scheduling
            .as_ref()
            .ok_or(crate::InputError::IncompleteSolution("scheduling output requires a scheduling"))?;
        ...
        for (choice_index, choice) in input.choices.iter().enumerate() {
            ...
            let slot_name = if slot == crate::Scheduling::NOT_SCHEDULED {
                ...
                input.slots[usize::try_from(slot).map_err(|_| {
                    crate::InputError::IncompleteSolution("scheduled choice referenced an invalid slot index")
                })?]
                    .name
                    .clone()
            };
    ...
    pub fn write_assignment_solution(solution: &Solution) -> crate::Result<FormattedOutput> {
        let scheduling = solution
            .scheduling
            .as_ref()
            .ok_or(crate::InputError::IncompleteSolution("assignment output requires a scheduling"))?;
        let assignment = solution
            .assignment
            .as_ref()
            .ok_or(crate::InputError::IncompleteSolution("assignment output requires an assignment"))?;
        ...
        for chooser in 0..input.chooser_count() {
            let mut choices = vec![0_usize; input.slot_count()];
            for slot in 0..input.slot_count() {
                let choice = assignment.choice_of(chooser, slot);
                let scheduled_slot = usize::try_from(scheduling.slot_of(choice)).map_err(|_| {
                    crate::InputError::IncompleteSolution("assignment output referenced an unscheduled choice")
                })?;
            ...
            for slot in 0..input.slot_count() {
                let mut choice_name = input.choices[choices[slot]].name.clone();
                if let Some(stripped) = choice_name.strip_prefix(crate::InputData::GENERATED_PREFIX) {
                    choice_name = stripped.to_owned();
                }
