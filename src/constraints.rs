...
impl Constraints {
    pub(crate) fn get_dependent_choices(constraints: &[Constraint], choice_count: usize) -> Vec<Vec<usize>> {
    ...
    pub(crate) fn reduce_and_optimize(constraints: &[Constraint], choice_count: usize) -> (Vec<Constraint>, bool) {
        ...
        for constraint in constraints {
            ...
            match constraint.kind {
                ...
                ConstraintType::ChoiceNotContainsChooser => new_kind = ConstraintType::ChooserIsNotInChoice,
            ...
            if add {
                reduced.push(Constraint::new(
                    new_kind,
                    if switch_sides {
                        usize::try_from(constraint.right).expect("constraint index must be non-negative")
                    } else {
                        constraint.left
                    },
                    if switch_sides {
        ...
        (Self::expand_dependent_constraints(&reduced, choice_count), is_infeasible)
    }

    fn expand_dependent_constraints(constraints: &[Constraint], choice_count: usize) -> Vec<Constraint> {
        let mut result = Vec::new();

        let dependent_choices = Self::get_dependent_choices(constraints, choice_count);
        let mandatory_critical_sets = Self::get_mandatory_critical_sets(constraints);
        ...
        for constraint in constraints {
            ...
            for dep_group in &dependent_choices {
                if dep_group.contains(&usize::try_from(constraint.right).expect("choice index must be non-negative")) {
        ...
        for constraint in constraints {
            if constraint.kind == ConstraintType::ChooserIsInChoice {
                groups
                    ...
                    .or_default()
                    .push(usize::try_from(constraint.right).expect("choice index must be non-negative"));
            }
        }
