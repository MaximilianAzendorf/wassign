...
local gs_path = os.getenv("GS") or "gs";
...
local function tikz2image(src, filetype, additional_packages)
  ...
  return with_temporary_directory("tikz2image", function (tmpdir)
    return with_working_directory(tmpdir, function ()
      ...
      local pdf_gs_file = file_template:format(tmpdir, "gs.pdf")
      ...
      -- Strip fonts
      pandoc.pipe(gs_path, {'-dNoOutputFonts', '-sDEVICE=pdfwrite', '-o', pdf_gs_file, pdf_file}, '')

      convert(pdf_gs_file, outfile)
...
function CodeBlock(block)
  ...
  local img_obj = pandoc.Image(caption, fname, title)

  -- Finally, put the image inside an empty paragraph. By returning the
  -- resulting paragraph object, the source code block gets replaced by
  -- the image:
  return pandoc.Div(pandoc.Para{ img_obj }, {class='diagram'});
end
