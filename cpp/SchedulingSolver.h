...
private:
    ...
    bool satisfies_critical_sets(map<int, int> const& decisions, vector<CriticalSet> const& criticalSets)
    {
        ...
        for(CriticalSet set : criticalSets)
        {
            coveredSlots.clear();
            int missing = 0;
...
public:
    ...
    SchedulingSolver(InputData const& inputData, bool doCriticalSetAnalysis = true)
        : _inputData(&inputData),
          _csAnalysis(*_inputData, doCriticalSetAnalysis),
          _hasSolution(false),
          _currentSolution(new Scheduling(inputData))
    {
    }

    void next_scheduling()
    {
