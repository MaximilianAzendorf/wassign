namespace WSolve
{
    ...
    {
        public Solution(InputData inputData, IEnumerable<(int workshop, int slot)> scheduling, IEnumerable<(int participant, int workshop)> assignment)
        {
            InputData = inputData;
        ...
        public void Verify()
        {
            ...
            void conductorsAreInOwnWorkshop(int w)
            {
                foreach (int conductor in InputData.Workshops[w].conductors)
                {
                    ...
                    {
                        throw new VerifyException($"Participant '{InputData.Participants[conductor]}', conductor of '{InputData.Workshops[w].name}', is not in his own workshop.");
                    }
                }
            }
            ...
            void oneWorkshopPerSlot(int p, int s)
            {
                ...
                if (c < 1)
                {
                    throw new VerifyException($"Participant '{InputData.Participants[p].name}' has no workshop in slot '{InputData.Slots[s]}'.");
                }

                if (c > 1)
                {
                    throw new VerifyException($"Participant '{InputData.Participants[p].name}' has more than one workshop in slot '{InputData.Slots[s]}'.");
                }
            }
